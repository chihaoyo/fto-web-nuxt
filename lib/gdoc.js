...
async function getDoc(publicURL) {
  ...
  for(let i = 0; i < tables.length; i++) {
    ...
    const { content: type, classes: cardClasses } = parseControls(cells.eq(0).text().trim()) // attributes: containerAttributes
      ...
      for(let a = 1; a < rows.length; a++) { // skip first row
        ...
        if(cols.length === 4) { // must have 4 languages
          ...
          for(let b = 0; b < 4; b++) {
            ...
            ml[loc] = col.text().trim().length > 0 ? html : defaultHTML
          }
          if(image) {
            slides.push({
              image,
        ...
        if(cols.length >= fields.length) {
          ...
          for(let b = 0; b < fields.length; b++) {
            ...
            if(field === 'avatar') {
              ...
              value = col.find('li').map((i, el) => $(el).text().trim()).toArray()
            }
            person[field] = value
          }
          if(person.id) {
